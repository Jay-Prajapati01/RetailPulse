...
def load_customer_intelligence() -> pd.DataFrame:
    ...
    if not personas.empty:
        persona_columns = [column for column in ["CustomerID", "persona", "Country", "kmeans_cluster", "dbscan_cluster", "pca_1", "pca_2"] if column in personas.columns]
        persona_frame = personas[persona_columns].rename(columns={"persona": "persona_label"}) if persona_columns else personas
        join_keys = [column for column in ["CustomerID"] if column in base.columns and column in persona_frame.columns]
        if join_keys:
...
def frame_to_metric_map(frame: pd.DataFrame) -> dict[str, float]:
    ...
    return {str(key): float(value) for key, value in first_row.items() if pd.notna(value)}
